...
function Index(options) {
  ...
  this.indexers = _.isArray(options.indexers)
    ? options.indexers
    : [options.indexers];
...
Index.prototype.create = function create(documents, options, callback) {
  ...
  function formatDocument(document) {
    return {
      ...
      fields: _.reduce(
        document,
        function (accumulator, values, key) {
          // rationalize input
...
Index.prototype.search = function search(query, options, callback) {
  ...
  // Map filters.
  options.filters = _.reduce(
    options.filters,
    function (filters, value, key) {
      ...
      // Append filter.
      if (_.isArray(ossFilter)) filters = filters.concat(ossFilter);
      // Array of filters.
  ...
  this.searcher.search(this.name, options, function (err, result) {
    ...
    if (index.formatters.output)
      result.documents = result.documents.map(index.formatters.output);
