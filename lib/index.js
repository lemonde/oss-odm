...
Index.prototype.search = function search(query, options, callback) {
  ...
  // Format query.
  options.query = formatQuery(options.query);
  ...
  options.joins = _.map(options.joins, function (join, name) {
    ...
    // Default query to '*:*'.
    join.query = formatQuery(join.query) || '*:*';
  ...
  /**
   * Format a query, filtering some special chars.
   *
   * @param  {String} query
   * @return {String}
   */
  ...
  function formatQuery(query) {
    ...
    // No negative lookbehind in javascript...
    return query.replace(/[:\()?!^*\[\]{}~]/g, function ($0, pos) {
      return ((query.slice(0, pos).match(/"/g) || []).length % 2) ? $0 : '';
    }).trim();
  }
};